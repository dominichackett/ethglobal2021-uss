...
const navigation = [
  ...
  { name: 'My Events', icon: CalendarIcon, current: false,href:'/myevents' },
  { name: 'Subscriptions', icon: HeartIcon, current: false,href:'/subscriptions' },

  { name: 'Earnings', icon: CashIcon, current: false,href:'/earnings' },
  { name: 'My NFTs',  icon: FilmIcon, current: false,href:'/mynfts' },
]
...
export default function DashBoardNav(props) {
  ...
  const profileMenu = () => {
    ...
    history.push(`/viewprofile/${user?.get("username")}`);
...
useEffect(() => {
  ...
  <Transition.Root show={sidebarOpen} as={Fragment}>
    <Dialog as="div" className="fixed inset-0 flex z-40 lg:hidden" onClose={setSidebarOpen}>
     ...
     >
       <div className="relative flex-1 flex flex-col max-w-xs w-full pt-5 pb-4 bg-my-green">
         <Transition.Child
           as={Fragment}
           ...
           <div className="px-2 space-y-1">
             {navigation.map((item) => (
               <Link
                 ...
                 to={item.href}
                 className={classNames(
                    location.pathname == item.href ? 'bg-my-green-alt5 text-white' : 'text-black hover:text-white hover:bg-my-green-alt6',
           ...
           <div className="mt-6 pt-6">
             <div className="px-2 space-y-1">
               {secondaryNavigation.map((item) => (
                 ...
                 >
                   <item.icon className="mr-4 h-6 w-6 text-my-green-light" aria-hidden="true" />
                   {item.name}
                 </a>
               ))}
             </div>
  ...
  {/* Static sidebar for desktop */}
   ...
   <div className="flex flex-col w-64">
     ...
     <div className="flex flex-col flex-grow bg-my-green pt-5 pb-4 overflow-y-auto">
  <div className="flex items-center flex-shrink-0 px-4 "  >
   ...
   Unstoppable Streams
       ...
       <nav className="mt-5 flex-1 flex flex-col divide-y divide-my-green-light overflow-y-auto" aria-label="Sidebar">
         <div className="px-2 space-y-1">
           {navigation.map((item) => (
             <Link
               key={item.name}
